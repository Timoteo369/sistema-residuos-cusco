...
async function crearAdminDirecto() {
  try {
    ...
    if (resultBusqueda.rows.length > 0) {
      console.log('El usuario ya existe. Actualizando su rol y contraseña...');
      await pool.query(
        'UPDATE usuarios SET rol = $1, contrasena = $2 WHERE correo = $3',
        ['administrador', contrasenaCifrada, correo]
      );
